...
- 周期任务的调度检查粒度必须明显小于识别间隔，配置为 120 秒时不能因粗粒度轮询变成约 180 秒。
- 识别能力必须保持插件式边界：内置 `screen-time` 插件提供服务和 Web 贡献，`el` CLI 与 Codex Plugin/MCP 只通过插件 HTTP 接口访问，独立管理页不嵌入书页。
...
- [ ] 配置识别间隔为 120 秒时，连续自动截图的实际间隔约为 120 秒，不因调度轮询被推迟到 180 秒。
- [ ] `el` CLI、Codex Plugin/MCP、书页插件入口、独立管理页和实际 daemon 均能读取或调用同一套屏幕识别能力，不出现第二套状态或密钥存储。
