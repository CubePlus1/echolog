...
The initial load may fetch the existing bounded history payload so hierarchy and parent links remain available. Subsequent live polls fetch `/records?limit=1` as a lightweight latest-record probe, `/records/active`, and `/summary/today`; a full history refresh is triggered when the probe or today's record count changes, or after an explicit write action.
