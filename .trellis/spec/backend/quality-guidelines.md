...
- 从隔离目录执行 `pnpm install` 后，不得直接归档完整安装树：`.modules.yaml`、`.pnpm-workspace-state-v1.json` 和 `.bin` shim 会记录 pnpm store 或临时 workspace 的绝对路径。ready-to-run 包应预编译迁移入口、裁剪为 production dependencies、删除这些本机元数据，并在归档前扫描 `/Users/*`、`/private/var/folders/*` 和绝对 symlink。
